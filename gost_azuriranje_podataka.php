...
if(!empty($novi_mejl)){
    if(!preg_match($regex_mejl, $novi_mejl)) {
        ...
        mysqli_query($con,"UPDATE gosti SET mejl='$novi_mejl' WHERE kor_ime='$kor_ime'");
        mysqli_query($con,"UPDATE korisnici SET mejl='$novi_mejl' WHERE kor_ime='$kor_ime'");
    }
}
if(!empty($novi_telefon)){
    mysqli_query($con,"UPDATE gosti SET telefon='$novi_telefon' WHERE kor_ime='$kor_ime'");
    mysqli_query($con,"UPDATE korisnici SET telefon='$novi_telefon' WHERE kor_ime='$kor_ime'");
}
if(!empty($nova_kartica)){
    if (!preg_match($regex_kartica, $nova_kartica)) {
        echo "<span style='color:red'>Uneli ste broj kartice u nepravilnom formatu!</span><br>";
    }else{
        mysqli_query($con,"UPDATE gosti SET kartica='$nova_kartica' WHERE kor_ime='$kor_ime'");
        mysqli_query($con,"UPDATE korisnici SET kartica='$nova_kartica' WHERE kor_ime='$kor_ime'");
    }
}
if(isset($_FILES['nova_slika']) && $_FILES['nova_slika']['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($ekstenzija, $dozvoljene_ekstenzije)) {
        ...
        if ($sirina >= 100 && $visina >= 100 && $sirina <= 300 && $visina <= 300) {
            ...
            mysqli_query($con, "UPDATE gosti SET slika='$nova_putanja' WHERE kor_ime='$kor_ime'");
            mysqli_query($con, "UPDATE korisnici SET slika='$nova_putanja' WHERE kor_ime='$kor_ime'");
        } else {
            echo "<span style='color:red'>Slika mora biti izmedju 100x100 i 300x300 piksela!</span><br>";
        }
    } else {
