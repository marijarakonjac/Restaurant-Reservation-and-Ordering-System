...
$postojeci_restoran = 0;
$id_restorana = 0;
$result = mysqli_query($con, "SELECT id FROM restorani WHERE naziv='$restoran'");
if (mysqli_num_rows($result) > 0) {
    $postojeci_restoran = 1;
    $row = mysqli_fetch_assoc($result);
    $id_restorana = $row['id'];
}

if(empty($kor_ime)||empty($lozinka)||empty($bez_pitanje)||empty($bez_odgovor)||empty($ime)||empty($prezime)||empty($pol)
        ||empty($adresa)||empty($telefon)||empty($mejl)||empty($restoran)){
...
else if(!$postojeci_restoran){
    echo "<span style='color:red'>Uneti restoran ne postoji!</span><br>";
}
    ...
    if (in_array($ekstenzija, $dozvoljene_ekstenzije)) {
        ...
        if ($sirina >= 100 && $visina >= 100 && $sirina <= 300 && $visina <= 300) {
            ...
            if(copy($putanja, $nova_putanja)) {
                ...
                mysqli_query($con, "insert into korisnici (kor_ime, lozinka, ime, prezime, pol, tip, bez_pitanje, bez_odgovor, adresa, telefon, mejl, slika) values ('$kor_ime','$lozinka_kript','$ime','$prezime','$pol','konobar','$bez_pitanje','$bez_odgovor','$adresa','$telefon','$mejl','$profilna_slika')");
                mysqli_query($con, "insert into konobari (kor_ime, lozinka, id_restorana, ime, prezime, pol, bez_pitanje, bez_odgovor, adresa, telefon, mejl, slika, status) values ('$kor_ime','$lozinka_kript','$id_restorana', '$ime','$prezime','$pol','$bez_pitanje','$bez_odgovor','$adresa','$telefon','$mejl','$profilna_slika','odobren')");
            } else {
